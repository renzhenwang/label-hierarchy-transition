...
data_dir='../data_target/Aircraft'   ## path to images
ratio=1.0                    ## relabeled ratio for semisupervised/semi-supervised setting
GPU_ID='0'                   ## device id
...
for seed in 10 100 1; do
    ...
    echo $seed
    out_dir='result/lht_naive@ratio_'${ratio}'_seed_'${seed}
    # snapshot=$out_dir'/model_best.pth.tar'
    CUDA_VISIBLE_DEVICES=${GPU_ID} python3 ./main_lht_naive.py \
        ${data_dir} \
        --ratio ${ratio} \
        --loss_type ${loss_type} \
        -b ${batch_size} \
