...
def arg_parse():
    ...
    parser.add_argument('--title', type=str, default='transition model for car recognition!')
...
def main():
    ...
    label_hierarchy = [num_families, num_classes]
...
def train(train_loader, model, optimizer, criterion, epoch, args):
    ...
    for i, (inputs, targets) in enumerate(train_loader):
        ...
        family_targets, species_targets = targets[:, 0], targets[:, 1]
        target_list = [family_targets.long(), species_targets.long()]
...
def validate(val_loader, model, args):
    ...
    for i, (inputs, targets) in enumerate(val_loader):
        ...
        # compute output
        (pred1_L1, pred1_L2), _ = model(inputs)

        # measure accuracy
        prec1_L1, prec5_L1 = accuracy(pred1_L1.data, family_targets, topk=(1, 5))
        prec1_L2, prec5_L2 = accuracy(pred1_L2.data, species_targets, topk=(1, 5))
    ...
    return top1_L1.avg, top1_L2.avg
...
def get_train_set(data_dir, args):
    ...
    train_data = CarDataset(data_dir, input_transform=train_data_transform, semi_level=True, ratio=args.ratio)
...
def get_test_set(data_dir, args):
    ...
    test_data = CarDataset(data_dir, input_transform=test_data_transform, semi_level=False, ratio=1.0)
